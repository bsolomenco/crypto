...
* 'configure.py --amalgamation --single-amalgamation-file --minimized-build --enable-modules=chacha,chacha_sse2,stream'
*
* Target
...
#define BOTAN_VERSION_DATESTAMP 20180410

#define BOTAN_VERSION_RELEASE_TYPE "release"

#define BOTAN_VERSION_VC_REVISION "git:d7d080992372cf4fbd569cce1d8cd6aa7599fa0d"
...
inline void xor_buf(uint8_t out[],
   ...
   for(size_t i = 0; i != length; ++i)
      out[i] = in[i] ^ in2[i];
#else
   for(size_t i=length; i-->0; out[i]=in[i]^in2[i]);
#endif
   }
